...
var IBSCswapPair = _interopDefault(require('@bscswap/contracts/build/IBSCswapPair.json'));
...
(function (ChainId) {
  ...
  ChainId[ChainId["BSC_MAINNET"] = 2000] = "BSC_MAINNET";
...
var FACTORY_ADDRESS = '0x9Ba9d4a543108BdcB573ED37Cf59142cfc1124Db';
var INIT_CODE_HASH = '0x89129e4860d92e05cdcc89beeb0e9be73821075d3e70fc0772d0d92d8dd6efe1';
...
Currency.ETHER = /*#__PURE__*/new Currency(18, 'WDOGE', 'WDOGE');
...
var Pair = /*#__PURE__*/function () {
  function Pair(tokenAmountA, tokenAmountB) {
    ...
    this.liquidityToken = new Token(tokenAmounts[0].token.chainId, Pair.getAddress(tokenAmounts[0].token, tokenAmounts[1].token), 18, "BLP-V2", "Bswap V2");
  ...
  Router.swapCallParameters = function swapCallParameters(trade, options) {
    ...
    switch (trade.tradeType) {
      case exports.TradeType.EXACT_INPUT:
        if (etherIn) {
          methodName = useFeeOnTransfer ? "swapExactWDOGEForTokensSupportingFeeOnTransferTokens" : "swapExactWDOGEForTokens"; // (uint amountOutMin, address[] calldata path, address to, uint deadline)

          args = [amountOut, path, to, deadline];
          value = amountIn;
        } else if (etherOut) {
          methodName = useFeeOnTransfer ? "swapExactTokensForWDOGESupportingFeeOnTransferTokens" : "swapExactTokensForWDOGE"; // (uint amountIn, uint amountOutMin, address[] calldata path, address to, uint deadline)
      ...
      case exports.TradeType.EXACT_OUTPUT:
        ...
        if (etherIn) {
          methodName = 'swapWDOGEForExactTokens'; // (uint amountOut, address[] calldata path, address to, uint deadline)

          args = [amountOut, path, to, deadline];
          value = amountIn;
        } else if (etherOut) {
          methodName = 'swapTokensForExactWDOGE'; // (uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
...
var Fetcher = /*#__PURE__*/function () {
  ...
  Fetcher.fetchPairData = function fetchPairData(tokenA, tokenB, provider) {
    try {
      ...
      return Promise.resolve(new contracts.Contract(address, IBSCswapPair.abi, provider).getReserves()).then(function (_ref) {
